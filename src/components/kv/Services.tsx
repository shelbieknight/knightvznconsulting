...
const services = [
  ...
  },
  {
    ...
    desc: "Targeted outreach, lead scoring, nurture sequences, and CRM sync — one end-to-end pipeline.",
        ...
        <div>
          ...
          <AnimatedSection delay={0.1}>
            <h2 className="font-display text-[clamp(40px,4.5vw,64px)] font-bold leading-[0.98] tracking-[-0.03em] text-primary-foreground">
              ...
              <br /><span className="text-pop">manual work.</span>
            </h2>
          </AnimatedSection>
        </div>
        ...
        {services.map((svc, i) => (
          <AnimatedSection key={svc.num} delay={0.05 * i}>
            <div className="bg-primary-foreground/[0.04] border border-primary-foreground/[0.08] p-9 lg:p-10 relative overflow-hidden group cursor-default hover:border-pop/30 hover:bg-primary-foreground/[0.06] transition-all duration-300 h-full card-lift">
              {/* Accent line */}
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-pop scale-x-0 origin-left group-hover:scale-x-100 transition-transform duration-500" />

              <span className="font-mono text-[12px] text-pop font-bold tracking-[0.15em] mb-7 block">
                {svc.num}
              </span>
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {svc.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[10px] tracking-[0.12em] uppercase font-semibold text-primary-foreground/40 border border-primary-foreground/[0.12] px-3 py-1 group-hover:text-pop group-hover:border-pop/30 transition-colors"
                  >
                    {tag}
                  </span>
                ))}
